...
export default function CreateBookForm() {
  ...
  const handleSubmit = (event) => {
    ...
    bookDaoRef.current
      ...
      .then((res) => {
        errors.length = 0;
        alert(t("form.notification.created"));
        navigate("/books");
      })
      .catch((error) => {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          <option value="">{t("form.book.placeholder.author")}</option>
          {authors.map((author) => (
        ...
        >
          <option value="">{t("form.book.placeholder.genre")}</option>
          {genres.map((genre) => (
